...
font = pygame.font.Font('shikha_code/font/myFont.ttf', 32)
big_font = pygame.font.Font('shikha_code/font/myFont.ttf', 60)
WIDTH = 900
HEIGHT = 800
...
two_coords = [[], [], []]
three_coords = [[], [], [], []]
menu_img = pygame.image.load(f'shikha_code/menus/mainMenu.png')
game_over_img = pygame.image.load(f'shikha_code/menus/gameOver.png')
pause_img = pygame.image.load(f'shikha_code/menus/pause.png')
for i in range(1, 4):
    bgs.append(pygame.image.load(f'shikha_code/bgs/{i}.png'))
    banners.append(pygame.image.load(f'shikha_code/banners/{i}.png'))
    guns.append(pygame.transform.scale(pygame.image.load(f'shikha_code/guns/{i}.png'), (100, 100)))
    if i < 3:
        for j in range(1, 4):
            target_images[i - 1].append(pygame.transform.scale(
                pygame.image.load(f'shikha_code/targets/{i}/{j}.png'), (120 - (j * 18), 80 - (j * 12))))
    else:
        for j in range(1, 5):
            target_images[i - 1].append(pygame.transform.scale(
                pygame.image.load(f'shikha_code/targets/{i}/{j}.png'), (120 - (j * 18), 80 - (j * 12))))
...
pygame.mixer.init()
pygame.mixer.music.load('shikha_code/sounds/bg_music.mp3')
plate_sound = pygame.mixer.Sound('shikha_code/sounds/Broken plates.wav')
plate_sound.set_volume(.2)
bird_sound = pygame.mixer.Sound('shikha_code/sounds/Drill Gear.mp3')
bird_sound.set_volume(.2)
laser_sound = pygame.mixer.Sound('shikha_code/sounds/Laser Gun.wav')
